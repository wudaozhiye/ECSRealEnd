	...
	public class NetworkMsgHandler : IRoomMsgHandler, ILoginHandler
{
	...
	{
		Debug.Log(msg, Array.Empty<object>());
	}
	...
	{
		EventHelper.Trigger(EEvent.OnConnLogin, null);
		Log("OnConnLogin ");
	}
	...
	{
		EventHelper.Trigger(EEvent.OnPlayerReadyInRoom, new object[2]
		{
			userId,
			state
		});
	...
	{
		EventHelper.Trigger(EEvent.OnLeaveRoom, null);
	}
